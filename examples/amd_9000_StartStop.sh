#!/bin/bash
# start/stop script for AMD 9000 series GPU passthrough
# Compatible with DKVM menu system
#
# This script handles:
# - Dynamic driver detection and unbinding
# - Device-specific amdgpu driver loading/unloading for VGA devices
# - VFIO-PCI binding for passthrough devices
# - Setup of iGPU protection
#
# Example: AMD 9000 series GPU passthrough

# Helper function to get the current driver for a PCI device
get_current_driver() {
	local device=$1
...
# Helper function to check if device is a VGA device
is_vga_device() {
	local device=$1
	# shellcheck disable=SC2155  # Declare and assign separately - readability preferred for simple sysfs reads
...
# Helper function to unbind driver from device
unbind_driver() {
	local device=$1
	local driver=$2
...
# Helper function to bind driver to device
bind_driver() {
	local device=$1
	local driver=$2
