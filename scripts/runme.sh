...
apk update
apk upgrade
apk add ca-certificates wget util-linux bridge bridge-utils amd-ucode intel-ucode qemu-img@community qemu-hw-usb-host@community qemu-system-x86_64@dkvm ovmf@community qemu-hw-display-virtio-vga@community swtpm@community bash dialog bc nettle jq lvm2 e2fsprogs pciutils irqbalance hwloc-tools capstone@community libslirp@community vde2-libs@community || err "Cannot install packages"
